...
class Captcha(object):
    ...
    #@classmenthod修饰的方法函数需要通过cls参数传递当前的类对象，@classmenthod表示当前调用的类名，cls表示这个类本身。
    @classmethod
    def gene_text(cls):
    ...
    def __gene_line(cls,draw,width,height):
        ...
        #fill对函数和坐标轴之间的区域进行填充，用fill。

    # 用来绘制干扰点
    @classmethod
    def __gene_points(cls,draw,point_chance,width,height):
        chance = min(100, max(0, int(point_chance))) # 大小限制在[0, 100]
        #看成两个函数，max(0, int(point_chance))挑选出大于等于零的，min(100在挑选出小于等于100的。
